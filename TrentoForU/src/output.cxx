...
void write_stream(std::ostream& os, int width, int num, double impact_param,
    int ncoll, const Event& event, double spin_a, double tilt_a, double spin_b, double tilt_b) {
  using std::fixed;
...
///LiuQi change it
  os << setprecision(5)
     << setw(10) << fixed      << spin_a
     << setw(10) << fixed      << tilt_a
     << setw(10) << fixed      << spin_b
     << setw(10) << fixed      << tilt_b;
...
void write_text_file(const fs::path& output_dir, int width, int num,
    double impact_param, int ncoll, const Event& event, bool header, double spin_a, double tilt_a, double spin_b, double tilt_b) {
...
class HDF5Writer {
 public:
  ...
  void operator()(int num, double impact_param,
      int ncoll, const Event& event,double spin_a, double tilt_a, double spin_b, double tilt_b) const;
...
void HDF5Writer::operator()(int num, double impact_param,
  ...
  ///LiuQi change it
  hdf5_add_scalar_attr(dataset, "spin_a", spin_a);
  hdf5_add_scalar_attr(dataset, "tilt_a", tilt_a);
  hdf5_add_scalar_attr(dataset, "spin_b", spin_b);
  hdf5_add_scalar_attr(dataset, "tilt_b", tilt_b);
...
Output::Output(const VarMap& var_map) {
  ...
  if (!var_map["quiet"].as<bool>()) {
    writers_.emplace_back(
      ...
      double spin_a, double tilt_a, double spin_b, double tilt_b) {
        write_stream(std::cout, width, num, impact_param, ncoll, event,spin_a, tilt_a, spin_b, tilt_b);
      }
    );
  }
      ...
      writers_.emplace_back(
        [output_path, width, header](int num, double impact_param,
          ...
          write_text_file(output_path, width, num,
              impact_param, ncoll, event, header,spin_a, tilt_a, spin_b, tilt_b);
        }
      );
    }
  }
