...
NucleusPtr Nucleus::create(const std::string& species, double nucleon_dmin) {
  ...
  //LiuQi change it for special collision of U+U
  else if (species == "U2LiuQi")
    return NucleusPtr{new DeformedNucleusSpecialConfig{
      238, 6.86, 0.420, 0.265, 0.000, 0.000, 3.1416, 0.000 ,6.2832, nucleon_dmin
    }};
...
//LiuQi change it for special collision of U+U
DeformedNucleusSpecialConfig::DeformedNucleusSpecialConfig(
    std::size_t A, double R, double a, double beta2, double beta4, double tilt_angle_min, double tilt_angle_max, 
    double spin_angle_min,double spin_angle_max ,double dmin)
    : MinDistNucleus(A, dmin),
      R_(R),
      a_(a),
      beta2_(beta2),
      beta4_(beta4),
      rmax_(R*(1. + .63*std::fabs(beta2) + .85*std::fabs(beta4)) + 10.*a),
      tilt_angle_min_(tilt_angle_min), tilt_angle_max_(tilt_angle_max), 
      spin_angle_min_(spin_angle_min), spin_angle_max_(spin_angle_max),
      tilt_angle_(0), spin_angle_(0)
{}


double DeformedNucleusSpecialConfig::radius() const {
  return rmax_ - 7.*a_;
}


double DeformedNucleusSpecialConfig::deformed_woods_saxon_dist(
    double r, double cos_theta) const {
  auto cos_theta_sq = cos_theta*cos_theta;
  // spherical harmonics
  using math::double_constants::one_div_root_pi;
...
void DeformedNucleusSpecialConfig::sample_nucleons_impl() {
  // The deformed W-S distribution is defined so the symmetry axis is aligned
  // with the Z axis, so e.g. the long axis of uranium coincides with Z.
  //
  // After sampling positions, they must be randomly rotated.  In general this
  ...
  // "tilt" angle
  tilt_angle_=random::canonical<double>()*(tilt_angle_max_-tilt_angle_min_)+tilt_angle_min_; 
  spin_angle_=random::canonical<double>()*(spin_angle_max_-spin_angle_min_)+spin_angle_min_;
  const auto cos_a = std::cos(tilt_angle_);
  const auto sin_a = std::sqrt(1. - cos_a*cos_a);

  // "spin" angle
  const auto angle_b = spin_angle_;
  const auto cos_b = std::cos(angle_b);
  const auto sin_b = std::sin(angle_b);
